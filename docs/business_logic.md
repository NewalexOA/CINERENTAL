...
- Из `AVAILABLE`: можно перейти в `MAINTENANCE` или `RETIRED`
- Из `MAINTENANCE`: можно перейти в `AVAILABLE` или `RETIRED`
- Из `RETIRED`: переходы запрещены (конечное состояние)

Дополнительные правила:
- Нельзя списать оборудование, если есть активные бронирования
- Оборудование в статусе `MAINTENANCE` или `RETIRED` нельзя забронировать
...
- Поиск категорий осуществляется по точному совпадению названия (без учета регистра)
