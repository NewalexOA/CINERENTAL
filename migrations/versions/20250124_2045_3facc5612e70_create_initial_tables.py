...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('status', sa.Enum('available', 'rented', 'maintenance', 'broken', 'retired', 
          name='equipmentstatus'), nullable=False, default='available'),
    ...
    op.create_table(
        ...
        sa.Column('status', sa.Enum('active', 'blocked', 'archived', 
          name='clientstatus'), nullable=False, default='active'),
        sa.Column('notes', sa.String(1000)),
    ...
    op.create_table(
        'bookings',
        sa.Column('id', sa.Integer(), primary_key=True),
        sa.Column('client_id', sa.Integer(), sa.ForeignKey('clients.id', ondelete='RESTRICT')),
        ...
        sa.Column('booking_status', sa.Enum('pending', 'confirmed', 'active', 'completed', 'cancelled', 'overdue', 
          name='bookingstatus'), nullable=False, default='pending'),
        sa.Column('payment_status', sa.Enum('pending', 'partial', 'paid', 'refunded', 'overdue', 
    ...
    op.create_table(
        ...
        sa.Column('type', sa.Enum('contract', 'invoice', 'receipt', 'passport', 'damage_report', 'insurance', 'other', 
